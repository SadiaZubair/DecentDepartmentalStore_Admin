...
function PendingOrderDetails({ match }) {
  ...
  useEffect(() => {
    ...
    const saveChanges1 = (e) => {
      e.preventDefault();
      var db1=fire.firestore();
      console.log(details.buyerid,"oo",orderidd);
      db1.collection("Buyer-info ").doc(details.buyerid).collection('Orders').doc(orderidd).update({
				state:status
			}).then(function(){
    ...
    return (
        ...
        <ListGroup horizontal > 
        <button type="button" onClick={(e)=>{saveChanges(e);saveChanges1(e);}} style={{backgroundColor: '#0277BD',marginTop:'10%', marginRight:'10%', height:'fit-content', color: '#FFFFFF', width: '150px'}}  class="btn back-color rounded-pill">Confirm</button>
        <Snackbar
        anchorOrigin={{
          vertical: 'bottom',
          horizontal: 'left',
      ...
      />
        <button type="button" style={{backgroundColor: '#0277BD',marginTop:'10%', marginRight:'10%', height:'fit-content', color: '#FFFFFF', width: '150px'}}  class="btn back-color rounded-pill">Cancel</button>
